...
export type PathsOf<T, Prefix extends string = ''> = T extends object ? {
    [K in keyof T]: K extends string | number ? T[K] extends object ? T[K] extends any[] ? `${Prefix}${K}` | PathsOf<T[K], `${Prefix}${K}.`> : `${Prefix}${K}` | PathsOf<T[K], `${Prefix}${K}.`> : `${Prefix}${K}` : never;
}[keyof T] : never;
/**
 * 根据路径获取类型
 */
 ...
 * 是否为联合类型
 */
export type IsUnion<T, U = T> = [T] extends [never] ? false : T extends U ? [U] extends [T] ? false : true : false;
