 ...
 * Angular Store 创建器
 * 
 * 基于 @ngrx/signals，增强了缓存、持久化、性能监控等功能
 * 
 * @module create-store
 */
 ...
 * Store 选项
 */
export interface AngularStoreOptions<S = any, A = any> {
  /** Store 名称 */
  name: string
  /** 初始状态 */
  initialState: S
 ...
 * 增强的 Angular Store
 */
export interface EnhancedAngularStore<S = any, A = any> {
  /** 响应式状态 */
  state: ReturnType<typeof signal<S>>
  /** Actions */
  actions: A
  /** 缓存实例 */
  $cache: LRUCache
 ...
 * ```
 */
export function createAngularStore<S extends object = any, A = any>(
  options: AngularStoreOptions<S, A>
): EnhancedAngularStore<S, A> {
  const {
    name,
    initialState,
  ...
  if (shouldPersist) {
    ...
    if (saved) {
      try {
        ...
        console.error('[Angular Store] 恢复持久化数据失败:', error)
      }
    }
  }
