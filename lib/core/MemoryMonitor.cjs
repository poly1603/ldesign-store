...
class MemoryMonitor {
  constructor(config = {}) {
    ...
    this.performanceMonitor = performance.PerformanceMonitor.getInstance();
   ...
   */
  collectMemoryUsage() {
    const storePool$1 = storePool.StorePool.getInstance();
    const poolStats = storePool$1.getStats();
  ...
  performGarbageCollection() {
    const storePool$1 = storePool.StorePool.getInstance();
    storePool$1.setOptions({ maxIdleTime: 6e4 });
    const keysToDelete = [];
    ...
    setTimeout(() => {
      storePool$1.setOptions({ maxIdleTime: 3e5 });
    }, 1e3);
...
//# sourceMappingURL=MemoryMonitor.cjs.map
