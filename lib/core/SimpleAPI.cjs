...
function createSimpleStore(id, options) {
  const state = vue.reactive(typeof options.state === "function" ? options.state() : options.state);
  ...
  if (options.getters) {
    for (const [key, getter] of Object.entries(options.getters)) {
      getters[key] = vue.computed(() => getter(state));
    }
  }
  const actions = {};
  if (options.actions) {
  ...
  const store2 = {
    ...
    watch(source, callback) {
      return vue.watch(source, callback);
    },
    // 监听所有状态
    watchAll(callback) {
      return vue.watch(() => state, callback, { deep: true });
    },
    // 批量更新
    batch(updater) {
      ReactiveSystem.batchUpdate(updater);
    },
    // 事务操作
...
function defineSimpleStore(id, setup) {
  return pinia.defineStore(id, setup);
}
...
exports.$ = store;
exports.SimpleStoreRegistry = SimpleStoreRegistry;
...
exports.defineSimpleStore = defineSimpleStore;
exports.inferStoreType = inferStoreType;
exports.store = store;
exports.storeRegistry = storeRegistry;
//# sourceMappingURL=SimpleAPI.cjs.map
