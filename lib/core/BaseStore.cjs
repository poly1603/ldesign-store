...
class BaseStore {
  ...
  constructor(id, options) {
    ...
    this.$id = id;
    this._optimizer = new PerformanceOptimizer.PerformanceOptimizer({
      cache: options?.cache,
      // 缓存配置选项
      // 处理持久化配置，确保类型正确
  ...
  _initializeStore(options) {
    ...
    this._storeDefinition = pinia.defineStore(this.$id, {
      state,
      actions,
      getters
    });
  ...
  _getDecoratorMetadata() {
    ...
    if (!BaseStore._metadataCache.has(ctor)) {
      const metadata = Reflect.getMetadata(decorators.DECORATOR_METADATA_KEY, ctor) || [];
...
//# sourceMappingURL=BaseStore.cjs.map
