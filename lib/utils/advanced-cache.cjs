...
class AdaptiveCache {
  constructor(initialSize = 1e3, minSize = 100, maxSize = 1e4, targetHitRate = 0.8, defaultTTL = 5 * 60 * 1e3) {
    this.cache = new cache.LRUCache(initialSize, defaultTTL);
  ...
  resizeCache(newSize) {
    ...
    this.cache = new cache.LRUCache(newSize, defaultTTL);
...
class MultiLevelCache {
  constructor(l1Size = 1e3, l1TTL = 5 * 60 * 1e3, l2Storage, l2Prefix = "cache:", serializer = {
    ...
    this.l1Cache = new cache.LRUCache(l1Size, l1TTL);
