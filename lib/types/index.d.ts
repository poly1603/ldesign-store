 ...
 */
export type SafeGetterDefinition<T extends Record<string, any> = Record<string, any>, S extends Record<string, unknown> = Record<string, unknown>> = {
 ...
 * Store 配置选项
 */
export interface StoreOptions<TState extends StateDefinition = StateDefinition, TActions extends ActionDefinition = ActionDefinition, TGetters extends GetterDefinition = GetterDefinition> {
    id: string;
    state?: () => TState;
    actions?: TActions;
    getters?: TGetters;
 ...
 * Store 类的基础接口
 */
export interface IBaseStore<TState extends StateDefinition = StateDefinition, TActions extends ActionDefinition = ActionDefinition, TGetters extends GetterDefinition = GetterDefinition> {
    readonly $id: string;
 ...
 * 向后兼容的 BaseStore 接口
 */
export interface BaseStore<TState extends StateDefinition = StateDefinition, TActions extends ActionDefinition = ActionDefinition, TGetters extends GetterDefinition = GetterDefinition> extends IBaseStore<TState, TActions, TGetters> {
}
/**
 * Hook 返回类型
 */
export interface UseStoreReturn<TState extends StateDefinition = StateDefinition, TActions extends ActionDefinition = ActionDefinition, TGetters extends GetterDefinition = GetterDefinition> extends BaseStore<TState, TActions, TGetters> {
    state: Ref<TState>;
    getters: TGetters;
    actions: TActions;
}
